...
func (m ImportModel) renderCardGameSelector() string {
	...
	value := focusedStyle.Render(m.selectedCardGame.Name)
	return label + value
}
...
func (m ImportModel) renderSetsList() string {
	listStyle := lipgloss.NewStyle().
		...
		Height(20)
	var b strings.Builder
	b.WriteString(focusedStyle.Render("Sets") + "\n\n")
...
func (m ImportModel) renderActionsPanel() string {
	panelStyle := lipgloss.NewStyle().
		...
		Height(20)
	var b strings.Builder
	b.WriteString(focusedStyle.Render("Actions") + "\n\n")
	...
	for i, action := range actions {
		...
		if action.enabled {
			if i == m.actionCursor && m.focusOnActions {
				line = focusedStyle.Render(fmt.Sprintf("> %s", action.label))
			} else {
				line = blurredStyle.Render(fmt.Sprintf("  %s", action.label))
			}
		} else {
			line = lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Strikethrough(true).Render(fmt.Sprintf("  %s", action.label))
...
func (m ImportModel) renderImportProgress() string {
	...
	if m.importProgress.setID != "" {
		content.WriteString(focusedStyle.Render(fmt.Sprintf("Current: %s", m.importProgress.setID)) + "\n\n")
	} else {
		content.WriteString(focusedStyle.Render("Starting import...") + "\n\n")
	}
